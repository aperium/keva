...
init_posit <- tibble(id = c("abc","lmo","xyz","lol"),
                    value = c(234,644,456,848),
                    rank = c(1,2,3,4))
...
positions <- init_posit |>
  mutate(series = sapply(0:(nrow(init_posit)-1),\(x) (1-bins^(-1))^x),
...
for(m in 1:length(buys)) {
  for(n in n_i:length(sells)) {
    n_1 <- n
    if(buys[n] + sum(list) > 0) list <- c(list, sells[n])
  }
  list <- c(list,buys[m])
}
print(list)
