...
if (length(py_pacs) > 0) r_pacs <- c("reticulate",r_pacs)
if (length(r_pacs) > 0) r_pacs <- c("librarian",r_pacs)
if (length(r_pacs) > 0) {
  # install pak
  if(length(find.package("pak", quiet = TRUE))==0) install.packages("pak")
  # install all other r packages
  r_pacs |> unique() |> pak::pak(upgrade = TRUE)
...
if (length(py_pacs)>0) {
  if (length(find.package("reticulate", quiet = TRUE))==0) install.packages("reticulate", quiet = TRUE)
