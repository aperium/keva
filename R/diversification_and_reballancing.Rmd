...
calc_trades <- function(positions, targets) {
  ...
  sells <- joined |> filter(!str_equal(id,"SPAXX"), changes < 0) |> pull(changes) |> unlist() |> sort()
  buys <- joined |> filter(!str_equal(id,"SPAXX"), changes > 0) |> pull(changes) |> unlist() |> sort(decreasing = TRUE)
  hold <- joined |> filter(!str_equal(id,"SPAXX"), changes == 0) |> pull(changes) |> unlist()
