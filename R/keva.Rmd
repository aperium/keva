...
read_excels <- function(path, trim_at_blank_line = TRUE,  ...) foreach(i=1:length(path), .combine = add_case, .packages = "readxl", export = c(trim_at_blank_line,...)) %do% {
  read_excel(path[i],...) %>%
    if_else(trim_at_blank_line == TRUE, filter(., if_all(everything(),\(x) cumany(sjmisc::is_empty(x)))), .)
  }
