...
start_date <- "2010-01-01" |> ymd(tz = "America/New_York") |> as_datetime()
...
td_data_batched <- foreach(i = 1:batch_count(length(download_details$Symbol)), .combine = rbind) %do% {
  batch <- keep((1:batch_size(length(download_details$Symbol)))+batch_size(length(download_details$Symbol))*(i-1), \(x) (x <= length(download_details$Symbol)))
  batch_download <- slow_time_series(download_details |> filter(row_number() %in% batch) |> pull(Symbol),interval = interval,"raw", apikey = TWELVEDATA_API_KEY)
