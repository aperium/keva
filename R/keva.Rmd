...
cran_pacs <- c("tidyverse","writexl","fs","stringr","rlang","magrittr","stats","foreach", "doParallel", "parallel", "fs", "janitor", "stringr","lubridate","forcats", "purrr","multidplyr", "curl", "units", "readxl", "datawizard") #"rvest", "chromote","httr2", "forecast"
dev_pacs <- c() 
bioc_pacs <- c()
py_pacs <- c()
...
other_symbols <- data_path |> 
  dir_ls(regexp = "[\\/]Download_Results.*xlsx") |>
  read_xlsxs() |>
  filter(!is.na(`YTD (Daily)`)) |>
  distinct() |>
...
slow_multi_download <- slowly(multi_download, rate_backoff())
...
if(nrow(download_details) |> as.logical()) {
  download_details %<>% mutate(slow_multi_download(url_query, file_dest),
                               headers |> http_headers_to_columns())
  # Retry with wait times
  errors <- download_details |> filter(!str_detect(`HTTP/2`, "2[:number:]{2}"))
  while(errors |> nrow() > 0) {
    errors %<>% mutate(slow_multi_download(url_query, file_dest),
                       headers |> http_headers_to_columns())
    rows_update(download_details, errors)
    errors %<>% errors |> filter(!str_detect(`HTTP/2`, "2[:number:]{2}"))
  }
...
# 
# download_details$headers |> http_headers_to_columns() |>
#   filter(!str_detect(`HTTP/2`, "2[:number:]{2}")) |> nrow()
...
calc_trades_2 <- function(positions2, opporotunities, cash_reserve = 0.05, sell_buffer = 0.05) {
  ...
  buyPotentials <- joined |> 
    ...
    slice_max(order_by = opporotunity, n=3) |>
    mutate(proportion = rescale(opporotunity, to = c(0, sum(opporotunity)), range = c(0, max(opporotunity))),
...
calc_trades_2(positions2, opporotunities, cash_reserve = 0.05, sell_buffer = 0.05)
