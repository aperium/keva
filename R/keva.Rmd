...
```{r calctradesfun}
calc_trades <- function(positions, targets, cash_reserve = 0.05, max_change = 0.1, min_change = 0.001) {
  ...
  buys <- joined |> 
    ...
    mutate(changes = if_else(cumsum(changes) < sum(moneymarket$value), changes, pmax(0,sum(moneymarket$value)-cumsum(changes)+changes) |> pmin(changes)))
  sells <- joined |> 
    filter(!str_equal(Symbol,"SPAXX"), changes < 0) |>
...
positions |> 
  ...
  calc_trades(targets, cash_reserve = 0.1, max_change = 0.1, min_change = 0.0005)
