...
START_DATE <- "2020-01-01" |> ymd(tz = "America/New_York") |> as_datetime()
...
td_data_batched <- foreach(i = 1:batch_count(n), .combine = rbind) %do% {
  ...
  batch_download <- slow_time_series(batch_rows |> pull(Symbol), start_date = min(batch_rows$start_date), end_date = max(batch_rows$end_date), interval = INTERVAL,"raw", apikey = TWELVEDATA_API_KEY) #
  
  # download details
  batch_meta_file_data <- batch_download |> lapply(\(y) y |> as.data.frame() |> select(!contains("values")) |> distinct() |> rename_with(\(x) str_extract(x,"[:alnum:]*$") |> str_to_title()) |> mutate(across(everything(), as_string))) |> list_rbind()
