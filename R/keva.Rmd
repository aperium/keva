...
hist2 |> 
  ...
  geom_point(aes(y = `Adj Close`),size = 1e-0, alpha = 3/4) +
  geom_point(aes(x = predictionDate, y = prediction_long, fill = NULL), data = longPred, shape = 24) +
  geom_point(aes(x = predictionDate, y = prediction_short, fill = NULL), data = shortPred, shape = 25) +
  geom_abline(aes(slope = estimate_Date_long, intercept = `estimate_(Intercept)_long`, color = Symbol, fill = Symbol), data = longPred, linetype = "longdash", alpha = 2/4) +
  geom_abline(aes(slope = estimate_Date_short, intercept = `estimate_(Intercept)_short`, color = Symbol, fill = Symbol), data = shortPred, linetype = "dashed", alpha = 2/4) +
