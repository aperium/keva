...
cran_pacs <- c("tidyverse","writexl","fs","stringr","rlang","magrittr","stats","foreach", "doParallel", "parallel", "fs", "janitor", "stringr","lubridate","forcats", "purrr","multidplyr", "curl", "units", "forecast") #"rvest", "chromote","httr2"
dev_pacs <- c() 
bioc_pacs <- c()
py_pacs <- c()
...
ggplot(aes(x = Date, y = apyDelta, color = Symbol, fill = Symbol)) +
  geom_point(size = 1e-4, alpha = 0.25) +
