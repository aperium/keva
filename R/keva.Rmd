...
download_details <- download_paramaters |>
  ...
  filter((!file_exists) | (modification_time < lastTradingClose() + dhours(4)) | ((modification_time < lastTradingClose()) & (file_age > dminutes(10))))
...
hist2 |> 
  ...
  filter(as.duration(interval(Date, max(Date))) <= dyears(.5)) |>
