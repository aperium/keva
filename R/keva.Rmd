...
```{r downloadTDdata, eval=FALSE}
# API guide: https://support.twelvedata.com/en/articles/5203360-bulk-requests
# baseURL <- "https://api.twelvedata.com/time_series"


START_DATE <- "2024-08-01" |> ymd(tz = "America/New_York") |> as_datetime()
END_DATE <- now(tzone = "America/New_York") |> round_date()
INTERVAL <- "1day"
...
download_paramaters <- add_row(targets |> select(Symbol), positions |> select(Symbol)) |>
  ...
  mutate(path = path(hist_path_td, Symbol |> str_c(".csv")),
         ...
         path |> file_info(),
         file_age = interval(modification_time, now()) |> as.duration(),
         path |> read_csv(col_select = c("Datetime")) |> summarise(earliest = min(Datetime), latest = max(Datetime)),
         start_date = if_else(START_DATE < earliest, START_DATE, latest) |> as_date(),
         end_date = if_else(END_DATE > latest, END_DATE, earliest) |> as_date(),
         needs_update = (!file_exists) | ((start_date <= end_date)&((modification_time < lastTradingClose() + dhours(4)) | ((modification_time < lastTradingClose()) & (file_age > dminutes(1))) | (size <= fs_bytes("1K")))))
...
td_data_batched <- foreach(i = 1:batch_count(n), .combine = rbind) %do% {
  batch <- keep((1:batch_size(n))+batch_size(n)*(i-1), \(x) (x <= length(download_details$Symbol)))
  batch_download <- slow_time_series(download_details |> filter(row_number() %in% batch) |> select(Symbol, start_date, end_date) |> rename(sym = Symbol),interval = interval,"raw", apikey = TWELVEDATA_API_KEY)
  
  # download details
  batch_meta_file_data <- batch_download |> lapply(\(y) y |> as.data.frame() |> select(!contains("values")) |> distinct() |> rename_with(\(x) str_extract(x,"[:alnum:]*$") |> str_to_title())) |> list_rbind()
  
  # save files
  batch_download |> lapply(\(y) y |> as.data.frame() |> select(contains("meta")|contains("values")) |> rename_with(\(x) str_extract(x,"[:alnum:]*$") |> str_to_title()) |> write_csv(file = download_paramaters |> filter(Symbol == y$meta$symbol) |> pull("path")))
  
  batch_meta_file_data
}

 # left_join(download_paramaters)

# DOWNLOAD META DATA AND STATUS
# td_data[[1]] |> as.data.frame() |> select(!contains("values")) |> distinct() |> rename_with(\(x) str_extract(x,"[:alnum:]*$"))
download_results <- td_data |> lapply(\(y) y |> as.data.frame() |> select(!contains("values")) |> distinct() |> rename_with(\(x) str_extract(x,"[:alnum:]*$") |> str_to_title())) |> list_rbind() |> right_join(download_paramaters)

# SAVE AS FILES
# td_data[[1]] |> as.data.frame() |> select(contains("meta")|contains("values")) |> rename_with(\(x) str_extract(x,"[:alnum:]*$") |> str_to_title()) |> write_csv(file = download_results |> filter(Symbol == td_data[[1]]$meta$symbol) |> pull("path"))
td_data |> lapply(\(y) y |> as.data.frame() |> select(contains("meta")|contains("values")) |> rename_with(\(x) str_extract(x,"[:alnum:]*$") |> str_to_title()) |> write_csv(file = download_results |> filter(Symbol == y$meta$symbol) |> pull("path")))
