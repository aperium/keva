...
cran_pacs <- c("tidyverse","writexl","fs","stringr","rlang","magrittr","stats","foreach", "doParallel", "parallel", "fs", "janitor", "stringr","lubridate","forcats", "purrr","multidplyr", "curl", "units", "readxl", "datawizard", "quantmod", "xts", "zoo", "TTR", "tidyquant") #"rvest", "chromote","httr2", "forecast"
dev_pacs <- c() 
bioc_pacs <- c()
py_pacs <- c()
...
read_excels <- function(path, ...) foreach(i=1:length(path), .combine = add_case, .packages = "readxl") %do% read_excel(path[i],...)
...
slow_multi_download <- slowly(multi_download, rate_backoff(max_times = 5))
