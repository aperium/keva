...
calc_trades <- function(positions, targets, cash_reserve = 0.05, max_change = 0.1, min_change = 0.001, sell_buffer = 0.05, transfer = 0) {
  ...
  buys <- joined |> 
    ...
    ##mutate(changes = if_else(cumsum(changes) < sum(moneymarket$value)*(1-sell_buffer), changes, pmax(0,sum(moneymarket$value)*(1-sell_buffer)-cumsum(changes)+changes) |> pmin(changes))) ##TODO
  sells <- joined |> 
    filter(!str_equal(Symbol,"SPAXX"), changes < 0) |>
...
new_positions <- positions |> 
  ...
  calc_trades(targets, cash_reserve = 0.0375, max_change = 0.2, min_change = 0.0001, sell_buffer = 0.05, transfer = -5000) 
