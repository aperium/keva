...
td_data_batched <- foreach(i = 1:batch_count(n), .combine = rbind) %do% {
  ...
  batch_download <- slow_time_series(batch_rows |> pull(Symbol), start_date = min(batch_rows$start_date), end_date = max(batch_rows$end_date), interval = INTERVAL,"raw", apikey = TWELVEDATA_API_KEY) #TODO something is going wrong here
