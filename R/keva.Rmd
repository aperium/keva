...
longPred <- predictPriceSummary(hist2, dyears(.5), nextTradingClose() |> as.Date()) |>
...
oppoPred <- full_join(longPred,shortPred) |>
  select(Symbol, predictionDate, prediction_long, prediction_short, starts_with("estimate")) |>
...
hist2 |> 
  ...
  geom_abline(aes(slope = estimate_Date_short, intercept = `estimate_(Intercept)_short`, color = Symbol, fill = Symbol), data = oppoPred_t12, linetype = "dashed", alpha = 2/4) +
  facet_wrap(vars(Symbol), scales = "free_y") +
  theme_minimal() +
  # ggpubr::clean_theme() +
  theme(legend.position="none", text = element_text(size = tsize))
